...
impl Mather for MyMather {
    async fn add(
        ...
        let difference = aim.first_summand - aim.second_summand;

        let reply = AddOutputMessage { difference };
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let mut laddr = std::env::var("LADDR").unwrap_or("0.0.0.0:5001".to_string());
    if laddr == "" {
        laddr = "0.0.0.0:5001".to_string();
    }
